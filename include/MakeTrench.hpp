...
public:
  ...
  MakeTrench(PSPtrType passedDomain, const NumericType passedGridDelta,
             ...
             const NumericType passedLeftTaperingAngle = 0.,
             const NumericType passedRightTaperingAngle = 0.,
      ...
      : domain(passedDomain), gridDelta(passedGridDelta),
        ...
        leftTaperingAngle(passedLeftTaperingAngle),
        rightTaperingAngle(passedRightTaperingAngle),
        baseHeight(passedBaseHeight), periodicBoundary(passedPeriodicBoundary),
  ...
  void apply() {
    ...
    if (leftTaperingAngle || rightTaperingAngle) {
      ...
      const NumericType rightOffset =
          std::tan(rightTaperingAngle * rayInternal::PI / 180.) * trenchDepth;
      if constexpr (D == 2) {
        for (int i = 0; i < 4; i++) {
